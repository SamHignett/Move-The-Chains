...
import {
  getTopStatsForCategory,
  StatCategoryConfig,
} from '@/features/teams/utils/StatUtils';
import {
  DefensiveStatsTemplate,
...
const statCategories: Record<
  string,
  StatCategoryConfig<Record<string, number>>
> = {
  Defensive: {
    getStats: (team: TeamStats) => team.defensive,
    template: DefensiveStatsTemplate,
  },
  Fumbling: {
  ...
  return (
    <Grid container spacing={3} sx={{ flexGrow: 1 }}>
      ...
      {Object.entries(statCategories).map(([categoryName, config]) => {
        const topStats = getTopStatsForCategory(teamStats, config);
        return (
          <Grid
            key={categoryName}
            size={3}
